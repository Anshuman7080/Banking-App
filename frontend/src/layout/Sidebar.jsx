...
const nav = [
    ...
    { label: "Transfers", to: "/dashboard/transactions", icon: <Send className="h-4 w-4" /> },
...
export function SidebarTrigger({ variant = "toggle", className = "", children, "aria-label": ariaLabel }) {
    return (
        <button
            onClick={variant === "open" ? onOpen : onToggle}
            aria-label={ariaLabel ?? (variant === "open" ? "Open menu" : "Toggle menu")}
            className={
                className ||
                "inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-gray-200 bg-white text-gray-700 shadow-sm hover:bg-gray-50 dark:border-white/10 dark:bg-[#101113] dark:text-white dark:hover:bg-[#131416] md:hidden"
            }
        >
        ...
        <nav className="mt-2 px-2">
            {nav.map(({ label, icon, to }) => (
                <Link
                    ...
                    onClick={onClose}
                >
                    <span className="inline-grid h-9 w-9 place-items-center rounded-xl border border-gray-200 bg-white shadow-sm transition group-hover:shadow-md dark:border-white/10 dark:bg-black/40">
                        {icon}
                    </span>
                    <span>{label}</span>
                </Link>
        ...
        <div className="mt-auto p-3">
            ...
            <div className="mt-3">
                <Link
                    ...
                    className="inline-flex w-full items-center justify-center rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-95 dark:bg-white dark:text-black"
                    onClick={onClose}
                >
                    New Transfer
                </Link>
...
export function MobileSidebar() {
    ...
    return (
        <>
            ...
            />
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-72 -translate-x-full transform border-r border-gray-200 bg-white/95 backdrop-blur transition-transform duration-300 dark:border-white/10 dark:bg-[#0b0b0c]/90 md:hidden ${
                    open ? "translate-x-0" : ""
                }`}
            >
                <div className="flex h-16 items-center justify-between px-4">
                    ...
                    <button
                        onClick={onClose}
                        className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-700 shadow-sm hover:bg-gray-50 dark:border-white/10 dark:bg-white/5 dark:text-white"
